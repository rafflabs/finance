...
dati["AAPL"].loc["2006":].dropna().cummax().plot()
...
times2 = logret2.apply(pd.Series.first_valid_index) + pd.offsets.DateOffset(years=1)
for t in tickers:
    logret2[t].loc[:times2[t]] = np.nan
    cummin2[t].loc[:times2[t]] = np.nan
...
tm = t2.copy()
for t in tickers:
    d1[t] = dati[t].loc[t1[t]]
    d2[t] = dati[t].loc[t2[t]]
    temp = dati.index[dati.index.get_loc(t2[t]) + 1]
    mm[t] = np.max(dati[t].loc[t1[t]:temp])
    try:
        tm[t] = dati[t].loc[t1[t]:temp].idxmax()
    except:
        tm[t] = np.nan
pd.DataFrame({"primo dato valido":t1, "prezzo1":d1, "data massimo":tm, "massimo":mm, "va sotto soglia":t2, "prezzo2":d2})[:50]

sottoSoglia = (cummin2 <- soglia)
s3 = (cummin2 <- soglia) & (drawdown < 0)
j = 0
for t in tickers:
    print(t, j)
    j += 1
    for i in dati.index:
        if sottoSoglia[t].loc[i] and not (s3[t].loc[i]):
            cummin2[t].loc[i:] = (drawdown[t].loc[i:]).cummin()
            sottoSoglia[t] = (cummin2[t]<-soglia)
...
limite = 50
print("buy&hold:", round(strategia1[limite:].mean(), 1), "%  -  strategia 2:", round(strategia2[limite:].mean(), 1), "%  -  strategia 3:", round(strategia3[limite:].mean(),1), "%")

plt.xticks(range(-10, 50, 5))
plt.xlabel("Rendimento annualizzato %", fontdict = {'fontsize' : 24})
strategia1.plot(kind="hist", figsize=(25,15), range=[-10,50], bins=100, legend=True, label="buy&hold", fontsize=20, color="green")
strategia2.plot(kind="hist", figsize=(25,15), range=[-10,50], bins=100, legend=True, label="strategia 2", fontsize=20, color="red", alpha=0.8)
strategia3.plot(kind="hist", figsize=(25,15), range=[-10,50], bins=100, legend=True, label="strategia 3", fontsize=20, color="blue", alpha=0.6)
...
# USARE QUESTO PER AVERE LE CRYPTO
crypto = [["Bitcoin", "BTC-USD", 1], ["Ethereum", "ETH-USD", 1]]
tickersc = list(np.array(crypto)[:, 1])
nomic = list(np.array(crypto)[:, 0])

datic = yf.download(tickersc)["Adj Close"]
# rimetto a posto le colonne che non ho capito perché me le scombina
datic = datic.reindex(tickersc, axis=1)

datic["BTC-USD"].plot(figsize=(25,15))
datic["BTC-USD"].dropna().cummax().plot()

datic["ETH-USD"].plot(figsize=(25,15))
datic["ETH-USD"].dropna().cummax().plot()

datimaxc = datic.cummax()
drawdownc = (datic-datimaxc) / datimaxc
cumminc = drawdownc.cummin()
logretc = np.log(datic / datic.shift(1))
...
times2c = logret2c.apply(pd.Series.first_valid_index) + pd.offsets.DateOffset(years=1)
for t in tickersc:
    logret2c[t].loc[:times2c[t]] = np.nan
    cummin2c[t].loc[:times2c[t]] = np.nan 
...
strategia2 = 100 * (np.exp(((cummin2c <- soglia) * logret2c).sum() / timesc) - 1)
strategia2

t1 = logretc.apply(pd.Series.first_valid_index)
t2 = (cummin2c<-soglia).eq(True).idxmax()
d1 = datic.iloc[0].copy()
d2 = datic.iloc[0].copy()
mm = datic.iloc[0].copy()
tm = t2.copy()
for t in tickersc:
    d1[t] = datic[t].loc[t1[t]]
    d2[t] = datic[t].loc[t2[t]]
    temp = datic.index[datic.index.get_loc(t2[t]) + 1]
    mm[t] = np.max(datic[t].loc[t1[t]:temp])
    try:
        tm[t] = datic[t].loc[t1[t]:temp].idxmax()
    except:
        tm[t] = np.nan
pd.DataFrame({"primo dato valido":t1, "prezzo1":d1, "data massimo":tm, "massimo":mm, "va sotto soglia":t2, "prezzo2":d2})[:50]

sottoSogliac = (cummin2c<-soglia)
s3c = (cummin2c<-soglia) & (drawdownc < 0)
j = 0
for t in tickersc:
    print(t, j)
    j += 1
    for i in datic.index:
        if sottoSogliac[t].loc[i] and not (s3c[t].loc[i]):
            cummin2c[t].loc[i:] = (drawdownc[t].loc[i:]).cummin()
            sottoSogliac[t] = (cummin2c[t]<-soglia)
            s3c[t] = ((cummin2c[t]<-soglia) & (drawdownc[t] <0 ))
